...
from app.services.opportunity_report_service import build_report_rows, summarize_report_rows
...
async def my_opportunity_report(
    ...
    result = await db.execute(
        select(Opportunity)
        .where(Opportunity.owner_id == str(current_user.id))
        .options(selectinload(Opportunity.owner))
